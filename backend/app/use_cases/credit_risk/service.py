...
from app.use_cases.credit_risk.schemas import SplitEvaluation
from app.use_cases.credit_risk.training import evaluate_test, evaluation_payload_for_db
from app.utils.json_safe import sanitize_for_json

CREDIT_VAL_RESULT_TYPE = "credit_val_evaluation"
CREDIT_TEST_RESULT_TYPE = "credit_test_evaluation"
...
def get_credit_evaluations(session: Session) -> dict:
    val_result = _latest_processed_result(session, CREDIT_VAL_RESULT_TYPE)
    test_result = _latest_processed_result(session, CREDIT_TEST_RESULT_TYPE)
    ...
    def _bundle(result: ProcessedResult | None) -> dict | None:
        ...
        if evaluation is None:
            return None
        return {"run": run.model_dump(), "result": result.model_dump(), "evaluation": evaluation}
...
def _ensure_datasets_seeded(session: Session) -> None:
    for key in (DATASET_KEY_TRAIN, DATASET_KEY_VAL, DATASET_KEY_TEST):
        ...
        if row is None:
            raise HTTPException(status_code=409, detail="Credit Risk datasets are not seeded. Run npm run data:generate and npm run db:seed.")
...
def _evaluation_metrics(evaluation: SplitEvaluation) -> dict:
    return {
        ...
        "decline_count": sum(1 for item in evaluation.records if item.decision == "Decline"),
        "review_count": sum(1 for item in evaluation.records if item.decision == "Manual underwriting review"),
        "average_pd_probability": round(sum(item.pd_probability for item in evaluation.records) / len(evaluation.records), 4) if evaluation.records else 0.0,
        ...
        try:
            ...
            session.add(
                ProcessedResult(
                    ...
                    result_type=CREDIT_TEST_RESULT_TYPE,
            ...
            session.add(
                AuditEvent(
                    actor="Local Analyst",
                    action="credit_risk_test_completed",
                    entity_type="model_run",
                    entity_id=run.id,
                    metadata_json={"split": "test", "primary_score": test_evaluation.primary_score, "precision": test_evaluation.precision, "recall": test_evaluation.recall},
                )
            )
            session.commit()
...
def start_credit_test_run(session: Session) -> dict:
    ...
    enqueue_user_job(f"credit-test-{run.id}", lambda: _run_test_evaluation_task(run.id))
    return {"run_id": run.id, "status": "running"}


def get_credit_run_progress(run_id: str, session: Session) -> dict:
    run = session.get(ModelRun, run_id)
    ...
    return {"run_id": run_id, "status": progress.status if progress.status != "running" else run.status, "progress_percent": progress.progress_percent, "stage": progress.stage}


def get_credit_run_result(run_id: str, session: Session) -> dict:
    run = session.get(ModelRun, run_id)
    if run is None or run.use_case_slug != USE_CASE_SLUG:
        raise HTTPException(status_code=404, detail="Run not found.")
    ...
    result = session.exec(
        select(ProcessedResult).where(ProcessedResult.run_id == run_id, ProcessedResult.result_type == CREDIT_TEST_RESULT_TYPE)
    ).first()
